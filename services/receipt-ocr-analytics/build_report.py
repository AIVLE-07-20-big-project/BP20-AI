...
기존 expense_analysis.py / cost_analysis.py의 계산 함수를 그대로 재사용하고,
그 결과를 하나의 정적 HTML 파일로 조립한다. (에이전트/LLM 없이 순수 계산 + 템플릿)

리포트 기간을 3가지 중 하나로 선택할 수 있다:
    - monthly : 특정 월 1개월치 보고서
    - yearly  : 특정 연도 1년치 보고서 (+ 월별 그래프)
    - full    : 데이터 전체 기간 보고서 (+ 월별 그래프 + 분기별 그래프)

기존 4개 섹션(이상지출/예산초과/매입단가/원가율)은 세 보고서 모두 동일하게 유지되고,
연간·총기간 보고서에는 그래프 섹션이 추가된다.

사용법:
    python build_report.py --data-dir cafe_synthetic_data --report-type full
    python build_report.py --data-dir cafe_synthetic_data --report-type yearly --year 2025
    python build_report.py --data-dir cafe_synthetic_data --report-type monthly --year 2026 --month 6
"""
...
from expense_analysis import detect_expense_anomalies, check_budget_overage
from cost_analysis import detect_price_changes, calculate_cost_rates
...
def _svg_multi_line_chart(labels, series_dict: dict) -> str:
    ...
    series_dict: {품목명: [값 또는 None, ...]} (값이 None인 구간은 선을 끊어서 그림)
    """
    n = len(labels)
...
def render_purchase_price_index_section(receipts: pd.DataFrame, items: pd.DataFrame,
    ...
    품목마다 절대 단가 규모가 크게 달라(원두 kg당 2만원대 vs 냅킨 장당 십원대) 그대로 겹쳐 그리면
    작은 품목은 안 보이게 되므로, 각 품목의 "첫 관측월 단가 = 100" 기준 지수로 환산해서 비교한다.
    """
...
def build_html_from_frames(receipts: pd.DataFrame, budget: pd.DataFrame, items: pd.DataFrame,
    ...
    (FastAPI 마이크로서비스처럼 파일이 아니라 API 요청 바디로 데이터를 받는 경우 사용)

    각 DataFrame은 기존 CSV 스키마와 동일한 컬럼을 가져야 한다:
        receipts: ReceiptID, StoreID, VendorName, TransactionDate(datetime), TransactionTime,
                   PaymentMethod, Category, SupplyAmount, Vat, TaxFreeAmount, TotalAmount
        budget:   YearMonth, Category, BudgetAmount, StoreID
        items:    ReceiptItemID, ReceiptID, ItemName, Quantity, Unit, UnitPrice, TotalPrice
        products: ProductID, StoreID, ProductName, Category, Price, DiscountRate
        orders:   OrderID, StoreID, OrderType, TotalAmount, DiscountAmount, PaymentMethod,
                   OrderedDate(datetime), OrderedTime
    """

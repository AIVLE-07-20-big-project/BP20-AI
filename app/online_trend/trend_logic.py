...
def get_supported_industries() -> list:
    """지원하는 업종 목록 반환 (API에서 사용, 대화형 입력 없음)"""
    return list(CANDIDATE_MENU_MAP.keys())


def get_candidates(industry: str) -> list:
    """업종명으로 후보 메뉴 리스트를 조회 (API에서 사용)"""
    if industry not in CANDIDATE_MENU_MAP:
        raise ValueError(f"'{industry}'는 지원하지 않는 업종입니다. "
                          f"지원 목록: {list(CANDIDATE_MENU_MAP.keys())}")
    return CANDIDATE_MENU_MAP[industry]
...
def build_report(industry: str) -> dict:
    ...
    candidates = get_candidates(industry)  # 지원하지 않는 업종이면 여기서 ValueError 발생
    ...
    html_report = generate_daily_report_html(
        industry, summary_df, skipped, start_date, end_date, long_term_trend
    )

...
1. 이미지 전처리 (EXIF 방향, 영수증 영역·원근·기울기 보정, 해상도 정규화, CLAHE+Sharpen — OpenCV)
...
### 상품 사전 fuzzy matching

`POST /api/v1/receipts/parse`에 선택적 multipart 문자열 필드 `productCatalog`를 JSON
배열로 전달하면 OCR 품목명을 상품 사전과 비교해 교정합니다. 전달하지 않으면 기존 OCR
품목명을 그대로 사용합니다.

```json
[
  {"name": "문어", "aliases": ["생문어", "돌문어"], "price": 12000},
  {"name": "3M 다용도 테이프 기획", "aliases": [], "price": 3800}
]
```

한글 자모·문자 유사도가 0.70 이상이고 1·2위 후보의 차이가 0.08 이상일 때만 교정하며,
가격 일치는 보조 점수로만 사용합니다. 최대 5,000개까지 전달할 수 있습니다.
...
- 실험용 `preparation`에서 검증한 CLAHE+Sharpen 전처리를 서비스 모듈로 통합했습니다.
- 영수증 외곽·원근·90도 방향·미세 기울기를 보정한 뒤 긴 변 1800px로 정규화합니다.
- 흐린 획 복원을 위해 약한 CLAHE+Sharpen을 2회 반복합니다. 강한 단일 필터보다 획 뭉침을 줄이면서 대비를 높입니다.
- 가장 성능이 좋았던 전처리 후보에 OCR을 한 번만 수행해 기존 2회 추론보다 지연을 줄였습니다.
- bbox 행 복원은 행 중앙값을 사용해 서로 다른 여러 줄이 연쇄 병합되는 문제를 방지합니다.
...
- 180도 뒤집힌 이미지, 심한 구김·반사·초점 불량 이미지는 재촬영 안내가 필요합니다.

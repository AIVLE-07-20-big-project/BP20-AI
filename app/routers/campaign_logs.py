...
from app.services.response.campaign_logs import DecisionNotApproved, DecisionNotFound, append_log, validate_logs

router = APIRouter()


@router.post("/campaign-logs")
def create_campaign_log(payload: CampaignLogRequest) -> dict:
    try:
        return append_log(
